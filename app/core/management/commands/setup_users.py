...
class Command(BaseCommand):
    def handle(self, *args, **kwargs):
        User.objects.create_user('eric', email='user1@kpkapp.biz', password='ericp123', first_name='Eric', last_name='Phillips')
        # User.objects.create_user('', email='user2@kpkapp.biz', password='', first_name='', last_name='')
        User.objects.create_user('saeed', email='user3@kpkapp.biz', password='saeeda123', first_name='Saeed', last_name='Abdus-Salaam')
        User.objects.create_user('marquette', email='user4@kpkapp.biz', password='marqp123', first_name='Marquette', last_name='Phillips')
        # User.objects.create_user('', email='user5@kpkapp.biz', password='', first_name='', last_name='')
        User.objects.create_user('grady', email='user6@kpkapp.biz', password='gradyw123', first_name='Grady', last_name='Wingfield')
