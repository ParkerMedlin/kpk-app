...
def _parse_rows(row_iter) -> Dict[str, Dict[str, Decimal]]:
    ...
    for idx, row in enumerate(row_iter):
        ...
        item_code = _get_cell_value(row, required_columns.get(ITEM_HEADER))
        if not item_code:
            continue
...
def _resolve_required_columns(headers: Dict[int, str]) -> Dict[str, int]:
    ...
    try:
        return {
            ITEM_HEADER: header_lookup[ITEM_HEADER],
            EST_LANDED_HEADER: header_lookup[EST_LANDED_HEADER],
            NEXT_COST_HEADER: header_lookup[NEXT_COST_HEADER],
        }
    except KeyError as exc:
        missing = ', '.join(
            header.title() for header in [ITEM_HEADER, EST_LANDED_HEADER, NEXT_COST_HEADER]
            if header not in header_lookup
        )
        raise PurchasingCostParseError(
            f"The workbook is missing required columns: {missing}."
        ) from exc
