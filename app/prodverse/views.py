...
def display_palletizer_camera(request):
    ...
    # Construct streaming URL based on the request host
    # This ensures the streaming URL points to the same host the user is accessing Django from
    host = request.get_host().split(':')[0]  # Get hostname without port
    stream_url = f'http://{host}:8889/hls/stream.m3u8'
