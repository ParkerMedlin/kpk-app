...
def GetSageTable(tblName):
    try:
        ...
        cnxnMAS = pyodbc.connect("DSN=SOTAMAS90;UID=parker;PWD=REDACTED_DB_PASSWORD;",autocommit=True) # odbc connection to get the data
        ...
        with open('init-db-imports\\dHeadLwithTypes\\'+tblName+'.txt', 'w') as f:
            f.write(dHeadLwithTypes)
        ...
        csvPathStr = 'init-db-imports'+'\\'+tblName+'.csv' # establish current user path for csv
        ...
        with open('init-db-imports\\dHeadLwithTypes\\'+tblName+'.txt') as f:
            dHeadLwithTypes = f.readlines()
        csvPathStr = 'init-db-imports'+'\\'+tblName+'.csv'

    cnxnPG = psycopg2.connect('postgresql://postgres:REDACTED_DB_PASSWORD@localhost:5432/blendversedb') # connection for dumping info into postgres
