...
if check_file_for_string(nginx_conf_path, "server app_blue:8001;"):
    ...
    # copy the file to the desktop as well for testing
    os.system(f"docker cp kpk-app_nginx_1:/etc/nginx/conf.d/nginx.conf {os.path.expanduser('~/Desktop/kpk-app')}")
    ...
    os.system(f"docker cp {nginx_conf_path} kpk-app_nginx_1:/etc/nginx/conf.d/nginx.conf")
    # copy the file to the desktop as well for testing
    os.system(f"docker cp kpk-app_nginx_1:/etc/nginx/conf.d/nginx.conf {os.path.expanduser('~/Desktop/kpk-app/nginx/nginx.conf')}")
    os.system("docker exec kpk-app_nginx_1 nginx -s reload")
    # os.remove(nginx_conf_path)
    print(f"Restarting...")
