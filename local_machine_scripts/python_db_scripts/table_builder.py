...
def create_tables():
    ...
    cursor_postgres.execute('update blend_bill_of_materials_TEMP set bill_desc=(select ci_item.itemcodedesc from ci_item where blend_bill_of_materials_TEMP.bill_pn=ci_item.itemcode);')
    cursor_postgres.execute("update blend_bill_of_materials_TEMP set foam_factor=1 where foam_factor IS NULL;")
    cursor_postgres.execute('drop table if exists blend_bill_of_materials')
    cursor_postgres.execute('alter table blend_bill_of_materials_TEMP rename to blend_bill_of_materials')
    ...
    cursor_postgres.execute("update prod_bill_of_materials_TEMP set ItemCodeDesc=CommentText where ItemCodeDesc='Default Item Code /C'")
    cursor_postgres.execute('drop table if exists prod_bill_of_materials')
    cursor_postgres.execute('alter table prod_bill_of_materials_TEMP rename to prod_bill_of_materials')
    ...
    cursor_postgres.execute('update blend_run_data_TEMP set adjustedrunqty=(unadjusted_runqty*1.1*foam_factor*qtyperbill)')
    ...
    cursor_postgres.execute('''alter table issue_sheet_needed_TEMP add batchnum1 text, add batchqty1 text, 
    ...
    for blend_pn in blend_pn_list:
        ...
        for item_number in range(9):
            ...
            cursor_postgres.execute("update issue_sheet_needed_TEMP set " + batch_num + "='" + batch_num_list[item_number] + "' where blend_pn='" + blend_pn + "'")
            cursor_postgres.execute("update issue_sheet_needed_TEMP set " + batch_qty + "='" + batch_qty_list[item_number] + "' where blend_pn='" + blend_pn + "'")
            batch_num = 'batchnum'
            batch_qty = 'batchqty'
    connection_postgres.commit()
    cursor_postgres.close()
    ...
    for tuple in tuple_list:
        part_number_list.append(tuple[0])
    alchemy_engine = create_engine('postgresql+psycopg2://postgres:REDACTED_DB_PASSWORD@localhost:5432/blendversedb', pool_recycle=3600)
    alchemy_connection = alchemy_engine.connect()
    timetable_df = pd.read_sql('select blend_pn, adjustedrunqty, oh_after_run, week_calc from timetable_run_data where oh_after_run<0', alchemy_connection)
    ...
    for part_number in part_number_list:
        ...
        cursor_postgres.execute("update blendthese_TEMP set one_wk_short=" + "'" + str(qty_this_blend_one_week) + "'" + " where blend_pn=" + "'" + part_number + "'")
        ...
        cursor_postgres.execute("update blendthese_TEMP set two_wk_short=" + "'" + str(qty_this_blend_two_weeks) + "'" + " where blend_pn=" + "'" + part_number + "'")
        ...
        cursor_postgres.execute("update blendthese_TEMP set three_wk_short=" + "'" + str(qty_this_blend_three_weeks)+"'" + " where blend_pn=" + "'" + part_number + "'")
    ...
    cursor_postgres.execute('''create table upcoming_blend_count_TEMP as
                                ...
                                ''')
    cursor_postgres.execute('alter table upcoming_blend_count_TEMP add column id serial primary key')
    cursor_postgres.execute('''DELETE FROM upcoming_blend_count_TEMP a USING upcoming_blend_count_TEMP b
