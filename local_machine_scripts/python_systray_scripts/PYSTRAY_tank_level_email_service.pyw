...
def get_rate_of_change_since_non_op_start(tank_name, debug=False):
    ...
    try:
        ...
        if debug:
            ...
            print(f"[DEBUG] {tank_name}: non_op_start={non_op_start}, rows={cnt}, first_ts={first_ts}, last_ts={last_ts}")
...
# Minimum hours into non-op period before we start checking
# This ensures we have enough non-op data to compare against
...
def check_all_tanks_for_leaks():
    ...
    2. Waits at least 2 hours into non-op period before checking
    ...
    if hours_in < MIN_HOURS_INTO_NON_OP:
        print(f"Skipping leak detection - only {hours_in:.1f}h into non-op period (need {MIN_HOURS_INTO_NON_OP}h)")
        return
...
def test_leak_detection():
    ...
    if is_non_operation_hours():
        ...
        hours_in = hours_into_non_op_period()
        print(f"Non-op period started: {non_op_start.strftime('%Y-%m-%d %H:%M')} ({hours_in:.1f}h ago)")
        print(f"Min hours required: {MIN_HOURS_INTO_NON_OP}h - {'MET' if hours_in >= MIN_HOURS_INTO_NON_OP else 'NOT MET'}")
    print()
    ...
    print("\n" + "=" * 60)
    print("TANK ANALYSIS")
    print("Control limits from: last 60 days non-op hours")
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if arg == '--test':
            ...
            print("Usage:")
            print("  python script.pyw           # Run as systray app")
            print("  python script.pyw --test    # Diagnostics (no emails)")
            print("  python script.pyw --test-email  # Send fake leak alert")
            print("  python script.pyw --force   # Run detection now (sends real emails)")
            return
